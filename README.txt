...
> To do the best fit policy, we decided to implement two separate lists.
	...
	“holeList_bySize”  - This holds the sorted hole list. We have this one so that we can easily find 
			     a hole for our best fit policy.
  The holeLists are both arrayLists of type “hole” - you can find our Hole/Segment/Page/Action objects at 
  the end of the file. (We made these as sub-classes because they only needed to be used by the
  MemoryManagement.java file.) Within our “Hole” object, we track the base and limit of each hole.

  We believe that having these two separate hole lists cuts down on the number of linear searches that we 
  would have to do through the list of holes if we were implement a singular hole list. 

  In order to keep our hole list sorted we implemented a merge sort. This felt like the most efficient
  decision for the amount of time that we were given.
